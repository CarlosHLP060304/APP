...
import 'schema/quiz_collection_record.dart';
...
export 'schema/quiz_collection_record.dart';
...
/// Functions to query QuizCollectionRecords (as a Stream and as a Future).
Future<int> queryQuizCollectionRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      QuizCollectionRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<QuizCollectionRecord>> queryQuizCollectionRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      QuizCollectionRecord.collection,
      QuizCollectionRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<QuizCollectionRecord>> queryQuizCollectionRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future maybeCreateUser(User user) async {
  final userRecord = QuizCollectionRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument =
        await QuizCollectionRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createQuizCollectionRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      QuizCollectionRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createQuizCollectionRecordData(email: email));
}
